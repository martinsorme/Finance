...
type Props = {project_id : number, selectedMembers:Dispatch<SetStateAction<{ value: number; label: string; }[]>>, selectUsers?: { value: number; label: string }[], task?: Task }

const AddMemberToNewTaskModal = ({project_id, selectedMembers, selectUsers, task}: Props) => {
    ...
    const handleUserClick = (user : { value: number; label: string }) => {
        ...
        selectedMembers((selectedUser) => [...selectedUser, user]);
  };
  const handleRemoveUser = (userToRemove : { value: number; label: string }) => {
    ...
    selectedMembers((prevSelectedUsers) =>
      prevSelectedUsers.filter((user) => user.value !== userToRemove.value)
    );
  };
  const handleRemoveAllUsers = () => {
    ...
    const saveMembers = async (task: number) => {
        let users = selectedUser.map((user) => user.value);
        try {
          const response = await fetch(`${BASE_URL}/specify_users_for_task/${task}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
    ...
    return (
      <div>
         <div style={{display: 'flex', width: "100%", marginTop: "10px"}}>
                <button style={{boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.2)',display: "flex", flexDirection: "row", justifyContent:"center", alignItems:"center", width: "150px", height: "40px", backgroundColor:"#E5EFF7", borderRadius:'5px', fontSize:'13px', }} onClick={openModal}>
                    <Icon icon="fluent:guest-add-24-filled" color="black" width="30" height="30" />
                    <div style={{fontSize:"12px", fontWeight:"bold" ,marginLeft:"5px"}}>Tilldela Uppgift</div>
        ...
        <Modal
          ...
          style={{
            ...
            content: { background: "white", display: "flex", flexDirection: "column", height: "430px", width: "350px", alignItems: "center", padding: "0px", marginTop: "150px", marginLeft: "565px", borderRadius:"5%" },
          }}
        >
          ...
          <div style={{display:"flex", height:"500px", width:"700px", flexDirection:"column", marginLeft:"400px", marginTop:"10px", overflowY: "scroll"}}>
           {userNames.map((user, index) => (
            <div key= {index} style={{display:"flex", flexDirection: "row", width:"300px", marginTop: "10px", alignItems:"left"}}>
...
export default AddMemberToNewTaskModal